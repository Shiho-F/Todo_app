...
### パスワード表示切り替え機能
ログイン画面およびサインアップ画面において、
入力したパスワードを確認できるよう
JavaScriptでパスワードの表示/非表示を切り替える機能を実装しました。

- 目のアイコンをクリックすることで表示状態を切り替え
- Django Formの`id_for_label`を利用して対象のinput要素を特定
- DOM操作により`type="password"` / `type="text"`を切り替えるシンプルな実装
- Bootstrap Iconsを使用し、表示状態が視覚的に分かるUIにしています。

開発中には、Docker環境において
staticファイルおよびテンプレートの反映が想定通り行われない問題に直面しましたが、
collectstatic の実行やコンテナの再ビルドを通じて原因を切り分け、解決しました。
この経験を通して、staticファイルとテンプレートの反映方法の違いについて理解を深めました。
